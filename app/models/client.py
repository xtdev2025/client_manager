...
class Client(User):
    ...
    @staticmethod
    def _prepare_plan_dates(
        plan_object_id,
        activation_input=None,
        expiration_input=None,
        ...
        try:
            ...
            if "plan_id" in data:
                if data["plan_id"]:
                    ...
                    force_update = (
                        plan_changed
                        or not existing_client.get("planActivatedAt")
                        or bool(plan_activation_input)
                        or bool(plan_expiration_input)
                    )
            ...
            result = mongo.db.clients.update_one({"_id": client_id}, {"$set": data})
