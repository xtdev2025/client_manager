...
class Domain:
    ...
    @staticmethod
    def create(name, cloudflare_api=None, cloudflare_email=None, cloudflare_password=None, 
               cloudflare_status=False, ssl=False, domain_limit=5):
        """Create a new domain"""
        try:
    ...
    def assign_to_client(client_id, domain_id, subdomain):
        ...
        try:
            ...
            if client_domains_count >= domain.get('domain_limit', 5):
                return False, f"Client has reached the domain limit of {domain.get('domain_limit', 5)}"
