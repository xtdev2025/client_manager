    ...
    try:
        ...
        if client and client.get("plan_id"):
            ...
            if client.get("expiredAt"):
                ...
                activation_date = (
                    client.get("planActivatedAt")
                    or client.get("updatedAt")
                    or client.get("createdAt")
                )
                if activation_date:
        ...
        for cd in client_domains:
            if cd.get("domain_id"):
                ...
                cd["domain"] = domain
                cd["full_domain"] = (
                    f"{cd['subdomain']}.{domain['name']}" if domain else cd["subdomain"]
                )
...
def change_password():
    ...
    try:
        ...
        return render_template(
            "client/my_change_password.html",
            user=client,
            plan=plan,
            now=datetime.utcnow(),
            navbar_plan_expiration=expiration_date,
